...
这个 skill 处理“从满意版本回写能力包”的动作。它不用于新建 skill；新建仍使用 `skill-creator`。它也不用于普通内容修改；用户明确要求沉淀，或适用的用户长期约定已将“认可 Skill 产出”设为迭代触发时接管。没有新的可复用反馈则不修改 Skill。
...
如果 `达尔文skill` 可用，读取它并按本次目标所需深度优化原 skill。明确反馈优先最小修复与相关回归，不为普通迭代强制多轮评分、逐阶段批准或成果卡。把本轮反馈作为输入，重点让达尔文处理：
